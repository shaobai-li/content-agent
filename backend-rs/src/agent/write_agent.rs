...
impl BaseAgent for WriteAgent {
    ...
    async fn handle_chat_stream(&self, ctx: AgentTurnContext) -> AgentStream {
        ...
        // 使用写作专用 system prompt 替换默认
        let system_msg = serde_json::json!({"role": "system", "content": WRITE_SYSTEM_PROMPT});
        // 查找并替换 system message
        let messages: Vec<Value> = if messages.first().map(|m| m.get("role") == Some(&Value::String("system".to_string()))).unwrap_or(false) {
...
mod tests {
    ...
    fn test_write_agent_config() {
        // Note: this test requires init_config() to be called first
