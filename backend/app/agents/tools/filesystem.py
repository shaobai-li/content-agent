...
from app.agents.tools.schema import BooleanSchema, IntegerSchema, StringSchema, tool_parameters_schema
from app.agents.tools import file_state
...
def _detect_image_mime(data: bytes) -> str | None:
    ...
    if data[:4] == b"II\x2a\x00" or data[:4] == b"MM\x00\x2a":
        return "image/tiff"
    if data[:4] == b"\x00\x00\x00\x0c" and data[4:8] == b"\x6a\x50\x20\x20":
        return "image/jp2"
    if data[:4] == b"\x00\x00\x00\x0c" and data[4:8] == b"\x6a\x50\x1a\x1a":
        return "image/jpx"
    return None


def _detect_image_mime_with_fallback(data: bytes, path: str) -> str | None:
...
class ReadFileTool(_FsTool):
    ...
    async def execute(
        ...
        pages: str | None = None, **kwargs: Any,
    ) -> Any:
        try:
            if not path:
            ...
            mime = _detect_image_mime_with_fallback(raw, path)
            if mime and mime.startswith("image/"):
                h = hashlib.sha256(raw).hexdigest()[:12]
                return f"(Image file: {path}, type={mime}, size={len(raw)} bytes, sha256={h})"
        ...
        try:
            if suffix == ".docx":
                ...
                try:
                    ...
                    return "Error: XLSX reading requires openpyxl. Install with: pip install openpyxl"
            ...
            return f"Error reading {suffix.upper()} file: {e}"

        if not text.strip():
            return f"({suffix.upper().lstrip('.')} has no extractable text: {fp})"
