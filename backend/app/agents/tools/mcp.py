...
Adapted from nanobot nanobot/agent/tools/mcp.py.
"""
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            available_wrapped_names = [
                _sanitize_name(f"mcp_{name}_{tool_def.name}")
            ...
            if any(
                ...
                hint = (
                    " Hint: this looks like stdio protocol pollution. Make sure the MCP server "
                    "writes only JSON-RPC to stdout and sends logs/debug output to stderr instead."
                )
