...
#
# Node deduplication: NetworkX G.add_node() is idempotent — calling it twice
# with the same ID overwrites attributes. Extractions merged via build_merge
# preserve existing nodes; new nodes with the same ID get updated attributes.
#
...
def _normalize_id(s: str) -> str:
    """Normalize an ID string for edge endpoint reconciliation."""
...
def build_from_json(extraction: dict, *, directed: bool = False) -> nx.Graph:
    ...
    # Normalized ID map: reconcile edge endpoints when the LLM generates IDs
    # with slightly different casing or punctuation than the node list.
...
def build(extractions: list[dict], *, directed: bool = False) -> nx.Graph:
    ...
    extraction's attributes win (NetworkX add_node overwrites).
    """
