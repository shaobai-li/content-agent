...
def save_to_knowledge_base(file_info: FileInfo, agent_id: str, kb_id: str):
    """将文件信息追加为 nodes.json 中的 record 节点"""
    kb_path = get_database_nodes_path(agent_id, kb_id)
    ...
    if kb_path.exists():
        ...
        if not isinstance(doc, dict):
            from app.service.knowledge_base_registry_service import ensure_kb_document
            doc = ensure_kb_document(agent_id, kb_id)
    else:
        from app.service.knowledge_base_registry_service import ensure_kb_document
        doc = ensure_kb_document(agent_id, kb_id)
