"""OpenAI-compatible provider for DeepSeek (and similar APIs)."""
...
def _safe_json_loads(text: str) -> dict[str, Any] | list[Any] | str | int | float | bool | None:
    """Parse JSON safely, returning a best-effort result.

    Tries ``json.loads`` first; if that fails, attempts a simple repair
    (wrap bare keys in quotes).  Returns ``{}`` for dict-typed callers when
    all parsing fails.
    """
    ...
    try:
        ...
        pass
    # Try wrapping unquoted keys (common LLM output issue).
    try:
        import re
        repaired = re.sub(
...
class OpenAICompatProvider(LLMProvider):
    """Unified provider for OpenAI-compatible APIs (DeepSeek, etc.)."""
    ...
    def _build_kwargs(
        ...
        thinking_active = (
            ...
            and reasoning_effort.lower() != "minimal"
        )
        if thinking_active:
    ...
    def _extract_usage(cls, response: Any) -> dict[str, int]:
        ...
        # cached_tokens: support DeepSeek's prompt_cache_hit_tokens
        for path in (
        ...
        return LLMResponse(
            ...
            tool_calls=[
                ToolCallRequest(
                    ...
                    name=b["name"],
                    arguments=_safe_json_loads(b["arguments"]) if b["arguments"] else {},
        ...
        *,
        ...
        if spec and spec.is_local and ("502" in text or "connection" in text or "refused" in text):
            msg += (
                ...
                f"{api_base or spec.default_api_base}."
            )
