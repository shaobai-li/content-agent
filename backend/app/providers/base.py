"""Base LLM provider interface — adapted from nanobot."""
...
# -- inlined from nanobot.utils.helpers --
...
class LLMResponse:
    ...
    reasoning_content: str | None = None  # DeepSeek-R1 etc.
    thinking_blocks: list[dict] | None = None  # Reserved for future use.
...
class LLMProvider(ABC):
    ...
    def _enforce_role_alternation(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        # recover by converting the last popped assistant message to a user
        # message so the LLM can still see the content.
        if (
            merged
