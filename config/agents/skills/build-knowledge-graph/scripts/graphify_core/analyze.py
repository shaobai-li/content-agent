...
def _is_file_node(G: nx.Graph, node_id: str) -> bool:
    """Return True if this node represents the source document itself."""
...
def god_nodes(G: nx.Graph, top_n: int = 10) -> list[dict]:
    """Return the top_n most-connected entities."""
...
def _is_concept_node(G: nx.Graph, node_id: str) -> bool:
    """Return True if this node is a manually-injected concept without a real source file."""
...
from ._file_constants import DOC_EXTENSIONS, PAPER_EXTENSIONS
...
def _surprise_score(
    ...
    # 2. Cross file-type bonus - paper↔doc is non-obvious
...
def _cross_file_surprises(G: nx.Graph, communities: dict[int, list[str]], top_n: int) -> list[dict]:
    """
    Cross-file edges between document entities, ranked by a composite
